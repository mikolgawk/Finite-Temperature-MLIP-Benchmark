...
[Back to the concise README](README.md).
...
- V2 has separate eager (`md_eager/`) and inference-accelerated
...
```text
paper_v2_configs/md_eager/model_calculators.json
paper_v2_configs/md_eager/md_calculator_versions.json
...
Both `paper_v2_configs/md_eager/` and `paper_v2_configs/md_accelerated/` run the same
...
`paper_v2_configs/md_eager/` provides the comparison workload without optional
...
- TorchSim output is written under
  `paper_v2_configs/data/mlip-trajs-torchsim-eager/`.
...
```bash
uv run --script paper_v2_configs/md_eager/torchsim-scripts/md_orb_v3.py
...
```bash
# Run every baseline TorchSim model sequentially.
CUDA_VISIBLE_DEVICES=0 bash paper_v2_configs/md_eager/ase-scripts/run_all_md.sh
...
- [`md_custom_model.py`](custom_model_evaluation/md_eager/ase-scripts/md_custom_model.py)
  for a baseline ASE `Calculator`;
- [`md_custom_model.py`](custom_model_evaluation/md_eager/torchsim-scripts/md_custom_model.py)
...
`paper_v2_configs/data/mlip-trajs-torchsim-eager`, respectively.
...
│   ├── mlip-trajs-torchsim-eager/   baseline TorchSim results
...
- the combined Pareto script defaults to the eager TorchSim outputs generated by
  this workflow; pass `--source mlip-trajs-torchsim-accelerated` to plot the
  accelerated outputs instead.
