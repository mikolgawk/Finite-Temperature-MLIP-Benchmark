...
# ]
# ///
"""TorchSim NVT production MD — orb-v2.
...
positions and velocities every RECORD_INTERVAL steps:
    ...
    <OUT_ROOT>/<system>/nvt_orb-v2.h5

Run:  uv run md_orb_v2.py
"""
...
SY_STEPS = 3                  # 3-term symmetric SY split per half-step, same structure as ASE; see README
RECORD_INTERVAL = 1           # save every Nth step
ACCELERATION = True           # torch.compile kernel fusion; same fp32 numerics
SEED = 20260724               # Maxwell-Boltzmann velocity seed
STATE_DTYPE = torch.float32

device = torch.device("cuda")

# model
# loaders return a model + adapter pair
orb_ff, adapter = pretrained.orb_v2(
    device=device,
