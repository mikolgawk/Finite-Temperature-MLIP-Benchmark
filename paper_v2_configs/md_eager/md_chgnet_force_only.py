...
same TorchSim HDF5 schema consumed by this repository's RDF analysis:
    ...
    <OUT_ROOT>/<system>/nvt_chgnet-force-only.h5
...
Run:  uv run md_chgnet_force_only.py
"""
...
def main() -> None:
    ...
    for name, meta in metadata.items():
        try:
            ...
            engine = "ase+chgnet-0.4.2-force-only"
            ...
            failure_out_csv = failure_out_dir / f"md_timing_{model_name}.csv"
            try:
                failure_out_dir.mkdir(parents=True, exist_ok=True)
                failure_out_csv.write_bytes(b"")
            except OSError as marker_error:
                print(
                    f"[{model_name}] {name}: could not write failure CSV: "
                    f"{marker_error}"
                )
            continue
