...
# dependencies = ["ase>=3.26", "h5py>=3.11", "mace-torch==0.3.16", "numpy>=1.26", "torch", "pandas>=2.2", "tqdm>=4.66"]
# [[tool.uv.index]]
# name = "pytorch-cu128"
...
def retain_mace_stress(calculator):
...
def make_calculator():
    ...
    if not CHECKPOINT.is_file():
        raise FileNotFoundError(f"MACE-MPA-0 checkpoint not found: {CHECKPOINT}")
    return retain_mace_stress(MACECalculator(
        str(CHECKPOINT), default_dtype="float32", device="cuda"
    ))


if __name__ == "__main__":
    run_ase_pressure("mace-mpa-0", make_calculator, "ase+mace-torch-0.3.16")
