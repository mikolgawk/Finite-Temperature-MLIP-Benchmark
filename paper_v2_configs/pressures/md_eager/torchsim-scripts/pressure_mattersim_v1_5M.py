...
# settings
MODEL_NAME = "mattersim-v1-5M-stress"
HERE = Path(__file__).resolve().parent
REPO = HERE.parents[3]
METADATA_FILE = REPO / "paper_v2_configs" /  "data" / "ref-trajs" / "md_metadata.json"
OUT_ROOT = REPO / "paper_v2_configs" / "data" / "mlip-trajs-torchsim-matched-stress"
...
# model
# and energy/force workload as the stress AOTI runner.
potential = Potential.from_checkpoint(
    load_path="MatterSim-v1.0.0-5M.pth", model_name="m3gnet",
    device=device, load_training_state=False,
)
model = WithStressTorchSimWrapper(model=potential, device=device)

run_torchsim_pressure(MODEL_NAME, model, "torch-sim-0.6.1+mattersim-eager-stress")
