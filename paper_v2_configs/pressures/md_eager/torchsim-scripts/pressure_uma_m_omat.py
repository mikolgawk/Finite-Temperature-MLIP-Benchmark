...
def load_with_stress_uma(model_name, device):
    ...
    calculator = FairChemModel(
        ...
        dtype=torch.float32, compute_stress=True,
    )
    return retain_uma_stress(calculator)
...
def main():
    model = load_with_stress_uma("uma-m-1p1", torch.device("cuda"))
    run_torchsim_pressure(
        "uma-m-omat-stress", model,
        "torch-sim-0.6.1+fairchem-2.21.0-stress",
    )
