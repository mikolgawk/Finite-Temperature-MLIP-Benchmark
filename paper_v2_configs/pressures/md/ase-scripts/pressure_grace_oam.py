...
def load_eager_grace(family):
    ...
    def compute(input_data):
        ...
        virial_matrix = tf.einsum("bi,bj->ij", pair_forces, data[C.BOND_VECTOR])
        virial = tf.stack([virial_matrix[0, 0], virial_matrix[1, 1],
                           virial_matrix[2, 2], virial_matrix[0, 1],
                           virial_matrix[0, 2], virial_matrix[1, 2]])[None, :]
        return {
            "virial": virial,
...
def make_calculator():
    ...
    calculator = WithStressTPCalculator(str(model_path), float_dtype="float32")
    calculator.stress_compute = load_eager_grace("grace_oam")
    calculator.compute_properties = ["energy", "forces", "stress"]
    return calculator

if __name__ == "__main__":
    run_ase_pressure("grace-oam", make_calculator, "ase+tensorpotential-stress")
