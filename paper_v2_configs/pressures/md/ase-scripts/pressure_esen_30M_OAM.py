...
"""Stress-enabled md_esen_30M_OAM.py: record potential stress for every saved trajectory frame."""
...
OUT_ROOT = REPO / "paper_v2_configs" / "data" / "mlip-trajs-torchsim-matched-stress"
...
class HDF5TrajectoryWriter:
    ...
    def __init__(self, filename: Path, atoms, model_name: str):
        ...
        self.stresses = data.create_dataset(
            "stress", shape=(0, 3, 3), maxshape=(None, 3, 3),
            chunks=(1, 3, 3), dtype=np.float64,
        )
        self.stresses.attrs["units"] = "eV/Angstrom^3"
        self.stresses.attrs["kind"] = "potential; tensile-positive; no kinetic term"

    def write(self) -> None:
        stress = np.asarray(self.atoms.get_stress(voigt=False), dtype=float)
        if stress.shape != (3, 3) or not np.isfinite(stress).all():
            raise ValueError("Invalid stress tensor; refusing an incomplete frame")
        ...
        self.stresses.resize(frame + 1, axis=0)
        self.stresses[frame] = stress
        self.file.flush()
...
def run_ase_md(
    ...
    if "stress" not in calculator.implemented_properties:
        raise RuntimeError("Pressure calculator must support stress")
    ...
    for name, meta in metadata.items():
        ...
        trajectory.file.attrs["timestep_fs"] = timestep_fs
        trajectory.file.attrs["record_interval"] = stride
        trajectory.file.attrs["first_step"] = 0
...
def retain_legacy_stress(calculator, family):
    """Keep the checkpoint stress heads and inference targets intact."""
    return None


CHECKPOINT = REPO / "paper_v2_configs/data/models/esen_30m_oam.pt"
...
def make_calculator():
    ...
    if not CHECKPOINT.is_file():
        raise FileNotFoundError(f"eSEN checkpoint not found: {CHECKPOINT}")
    calculator = OCPCalculator(
        checkpoint_path=str(CHECKPOINT), cpu=False, seed=42, disable_amp=True
    )
    retain_legacy_stress(calculator, "esen")
    return calculator


if __name__ == "__main__":
    run_ase_pressure("eSEN-30M-OAM", make_calculator, "ase+fairchem-core-1.10.0")
