...
def load_with_stress_legacy(checkpoint, family: str, device, seed: int = 42):
    ...
    calculator = FairChemV1Model(
        model=str(checkpoint), device=device, compute_stress=True,
        seed=seed, pbc=True, disable_amp=True,
    )
    retain_legacy_stress(calculator, family)
    return calculator
...
def main():
    ...
    model = load_with_stress_legacy(checkpoint, "esen", torch.device("cuda"))
    run_torchsim_pressure(
        "eSEN-30M-OAM-stress", model,
        "torch-sim-0.5.2+fairchem-1.10.0-stress",
    )
