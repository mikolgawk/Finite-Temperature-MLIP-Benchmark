#!/usr/bin/env python3
"""Correlate VDOS model mean error with pressure error.

Reads a VDOS summary CSV and a pressure error CSV, merges on model name,
and computes Pearson/Spearman correlations between VDOS error (%) and
pressure error (%).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Correlate VDOS error with pressure error")
    parser.add_argument("--source", choices=SOURCES, default=DEFAULT_SOURCE)
    parser.add_argument("--vdos-file", type=Path, default=None, help="Override the generated VDOS model summary CSV.")
    ...
    parser.add_argument("--output-csv", type=Path, default=SCRIPT_DIR / "results/vdos_pressure_merged_same_length.csv")
    parser.add_argument("--output-json", type=Path, default=SCRIPT_DIR / "results/vdos_pressure_correlation_same_length.json")
    parser.add_argument("--output-plot", type=Path, default=SCRIPT_DIR / "plots/correlation_SI_vdos_pressure_same_length.pdf")
...
def plot_scatter(merged: pd.DataFrame, output_plot: Path) -> None:
    ...
    y = merged["vdos_error_percent"].to_numpy(dtype=float)
    ...
    try:
        pearson = merged["vdos_error_percent"].corr(merged["pressure_error_percent"], method="pearson")
...
def main() -> None:
    args = parse_args()
    _, _, default_vdos = source_paths(args.source)
    backend, mode = source_pressure_provenance(args.source)
    vdos = load_vdos(args.vdos_file or default_vdos)
    ...
    merged = vdos.merge(pressure, on="model", how="inner")
    ...
    corr = {
        ...
        "pearson_error_vs_pressure": float(merged["vdos_error_percent"].corr(merged["pressure_error_percent"], method="pearson")),
        "spearman_error_vs_pressure": float(merged["vdos_error_percent"].corr(merged["pressure_error_percent"], method="spearman")),
    }
