...
# ///
"""Native ASE energy/force-only counterpart to torchsim-scripts/md_pet_oam_xl_force_only_compile.py.

Run: uv run ase-scripts/md_pet_oam_xl_force_only_compile.py
"""
...
def make_calculator():
    ...
    checkpoint = hf_hub_download("lab-cosmo/upet", "models/pet-oam-xl-v1.0.0.ckpt")
    model = _get_upet_exported_atomistic_model(
        model="pet-oam", size="xl", version="1.0.0", checkpoint_path=checkpoint
    )
...
if __name__ == "__main__":
    run_ase_md("pet-oam-xl", make_calculator, "ase+upet-0.2.6-torchscript-force-only")
