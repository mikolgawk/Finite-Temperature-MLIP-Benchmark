...
# ///
"""TorchSim NVT production MD — nequip.
...
CHECKPOINT = REPO / "updated_configs" / "data" / "models" / "mir-group__NequIP-OAM-XL__0.1.nequip.pth"
...
# model
# NequIP-OAM-XL, compiled TorchScript artifact, same checkpoint as the ASE benchmark.
# NOTE: NequIPFrameworkModel always runs internally at float64 and always computes
# stress (both hardcoded in nequip.integrations.torchsim.NequIPTorchSimCalc.__init__,
# confirmed against nequip 0.19.0 - compute_forces/compute_stress have no setters and
# there is no dtype kwarg here). It casts states given to it in other dtypes, so
# STATE_DTYPE is left at float32 for consistency with the rest of this catalog.
