...
# ///
"""TorchSim NVT production MD — eSEN-30M-OAM.

esen_30m_oam.pt is a legacy fairchem v1 checkpoint. The current
torch_sim.models.fairchem.FairChemModel (fairchem-core 2.x) cannot load it
(its loader requires the hydra MLIPInferenceCheckpoint format; the published
checkpoint is v1-only and no hydra version exists on facebook/OMAT24), so
...
with positions and velocities:
    ...
    <OUT_ROOT>/<system>/nvt_eSEN-30M-OAM.h5

Run:  uv run md_esen_30M_OAM.py
"""
...
# model
# eSEN-30M, OAM checkpoint (energy-conserving: forces are autograd of energy);
# same checkpoint as model_calculators.json's "eSEN-30M-OAM" entry.
# dtype is NOT passed: FairChemV1Model would inject a dtype key into the
# backbone/head configs and eSEN_Backbone.__init__ rejects it (TypeError).
# Omitting it changes nothing: _dtype defaults to float32, the checkpoint
# weights are natively float32, and disable_amp=True keeps autocast off.
model = FairChemV1Model(
    ...
    pbc=True,
    disable_amp=True,           # default; kept explicit for the record
)
