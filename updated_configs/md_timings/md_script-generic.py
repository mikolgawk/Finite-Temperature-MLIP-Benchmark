...
short warmed-up NVT MD benchmark using a calculator selected at runtime from
models_revised.json.
...
def nvt_simulation(init_structure, temperature, n_steps, time_step,
    ...
    '''Runs a warmed-up NVT molecular dynamics timing benchmark.'''
    ...
    # Warmup steps (untimed) to avoid measuring one-time CUDA kernel /
    # model-compilation costs in the production timing below.
    warmup_steps = max(1, round(NVT_WARMUP_FRACTION * n_steps)) if n_steps else 0
    if warmup_steps > 0:
        print(f"  Running {warmup_steps} warmup MD steps (untimed)...")
