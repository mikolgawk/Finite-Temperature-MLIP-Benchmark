...
# ///
"""Compiled native ASE energy/force-only counterpart to torchsim-scripts/md_esen_30M_OAM_force_only_compile.py.

Run: uv run ase-scripts/md_esen_30M_OAM_force_only_compile.py
"""
...
def make_calculator():
    ...
    disable_legacy_stress(calculator, "esen")
    calculator.trainer.model = torch.compile(
        calculator.trainer.model, dynamic=True, fullgraph=False
    )
    return calculator


if __name__ == "__main__":
    run_ase_md("eSEN-30M-OAM", make_calculator, "ase+fairchem-core-1.10.0+compile+force-only")
