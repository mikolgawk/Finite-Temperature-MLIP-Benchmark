...
# ///
"""Compiled force-only TorchSim NVT production MD — grace-oam.
...
with positions and velocities:
    ...
    <OUT_ROOT>/<system>/nvt_grace-oam-force-only-compiled.h5
...
class GraceModel(ModelInterface):
    ...
    def __init__(self, model, device=None, dtype=torch.float32,
        ...
        if compute_stress:
            raise ValueError("This compiled runner supports energy and forces only")
        self._compute = prune_grace_stress(self._tp.compute)
...
def main():
    ...
    run_torchsim_md(MODEL_NAME, model, "torch-sim-0.6.1+tf-compiled-dlpack-force-only", validate=None)
