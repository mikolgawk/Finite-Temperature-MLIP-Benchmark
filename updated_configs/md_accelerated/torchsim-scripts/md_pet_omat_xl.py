...
# ///
"""TorchScript force-only TorchSim NVT production MD — pet-omat-xl.
...
with positions and velocities:
    ...
    <OUT_ROOT>/<system>/nvt_pet-omat-xl-force-only-torchscript.h5

Run:  uv run md_pet_omat_xl.py
"""
...
def main():
    # settings
    MODEL_NAME = "pet-omat-xl-force-only-torchscript"
    ...
    model = MetatomicModel(
        model=get_upet(model="pet-omat", size="xl", version="1.0.0"),
        device=device, compute_stress=False,
    )

    run_torchsim_md(MODEL_NAME, model, "torch-sim-0.6.1+pet-torchscript-force-only")
