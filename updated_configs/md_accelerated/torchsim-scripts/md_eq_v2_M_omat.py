# /// script
# requires-python = ">=3.12,<3.13"  # Match eSEN's Python requirement for torch 2.4.1+cu124.
# dependencies = [
...
# ///
"""Force-only compiled TorchSim MD for eq-v2-M-omat; original checkpoint weights."""
...
def main():
    checkpoint = Path(__file__).resolve().parents[2] / "data" / "models" / "eqV2_86M_omat_mp_salex.pt"
    ...
    run_torchsim_md(
        "eq-v2-M-omat-force-only-compile", model,
